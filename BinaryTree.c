    ...
    if (*Node == NULL) {
        ...
        *Node = (struct node *) malloc (sizeof(struct node));
        ...
        if (*Node != NULL) {
            ...
            printf("Scuze boss, nu vreau satz dau memorie (sami sugi pula)");
            exit (-1);
        }
...
struct node *deleteNode(struct node *Node, int value) {
    ...
    //finding that goddamn node to be deleted
    if(value < Node->data)
    ...
    // node to be deleted has two babies
    else {
    ...
    // node to be deleted has one baby or none
        else {
